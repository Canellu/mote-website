...
The desktop composition is editorial and asymmetric after a centered opening sequence. The sticky header spans the viewport while its contents align to a 76rem grid; story sections can expand to 86rem and readable utility pages stop at 44rem. The opening message is centered above a partially revealed Mote window; scrolling enlarges that real product view and resolves three concise dashboard facts around it. Dashboard, settings, and Sync Box evidence then alternate copy and media. A second evidence gallery holds room controls, the scene library, and two light-placement modes in one large changing surface; the three related widget stories likewise consolidate into one changing media surface with a stacked selector.
...
At 64rem and below, the hero typography and callout measures tighten. At 50rem and below, the header compacts, the secondary brand word becomes visually hidden, the hero callouts move into a row beneath the image, all major sections become one column, scene options become a two-column grid, capture stories stop alternating, and the widget showcase places its active image before the stacked selector. At 35rem and below, the hero returns to left alignment, its callouts stack, navigation and panel padding tighten, and calls to action stack. The page remains usable from a 320px viewport.

### Hero Focus Sequence

The opening copy is deliberately more compact than the product evidence beneath it. The image begins partially below the fold at 78% scale inside a wide stage that can extend beyond the ordinary content column, so the initial viewport shows only its upper portion. It grows smoothly to its natural size during the first phase of document scroll, then holds in a centered sticky stage. Only after the image is fully focused do three short, factual dashboard notes resolve in the surrounding space. They use the application's compact Geist typography without connector lines or overlaying the interface. Reduced-motion and compact-layout visitors see the natural-size image and all facts without spatial motion.
